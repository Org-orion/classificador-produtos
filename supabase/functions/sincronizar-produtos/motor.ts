// =============================================================================
// CÓPIA Deno de src/lib/motor-classificacao.ts (paridade via
// src/test/paridade-edge.test.ts). Não editar uma sem a outra. Módulo puro.
// =============================================================================
import { hashDescricao, type DescricaoNormalizada } from './normalizacao.ts';
...
export const TETO_CONFIANCA: Record<Origem, number> = {
  CODIGO_APROVADO: 1.0, REGRA_CODIGO: 1.0, REGRA_DESCRICAO: 0.98,
  PARSER: 0.99, SIMILARIDADE: 0.94, IA: 0.89, MANUAL: 1.0,
};
...
export interface Regra {
  id: string; nome: string; familia: string | null;
  condicoes: CondicaoGrupo; resultados: Record<string, string>;
  prioridade: number; confianca: number; interromper: boolean; ativo: boolean;
}

export interface AtributoClassificado {
  valor: string; confianca: number; origem: Origem; evidencias: string[]; regra_id: string | null;
}
...
export interface ProdutoParaClassificar { codigo: string | null; descricao: DescricaoNormalizada; }
export interface ClassificacaoAprovada {
  hash_descricao: string; familia: string | null; atributos: Record<string, AtributoClassificado>;
}
export interface OpcoesClassificacao {
  aprovadaPorCodigo?: ClassificacaoAprovada;
  opcoesPermitidas?: Record<string, string[]>;
  atributosObrigatorios?: string[];
  versaoMotor?: string;
}
...
function comoLista(v: string | string[]): string[] { return Array.isArray(v) ? v : [v]; }
...
export function classificar(
  ...
  if (aprovada && aprovada.hash_descricao === hashDescricao(produto.descricao.normalizada)) {
    return {
      familia: aprovada.familia, atributos: aprovada.atributos, confianca_global: 1.0,
      status: 'APROVADO_AUTOMATICAMENTE', origem_global: 'CODIGO_APROVADO',
      regras_aplicadas: [], regras_rejeitadas: [], conflitos: [], evidencias: [], motivo_bloqueio: null,
    };
  }
  ...
  for (const regra of ativas) {
    ...
    for (const [attr, val] of Object.entries(regra.resultados)) {
      ...
      if (travado[attr]) continue;
      if (atual.valor === val) continue;
  ...
  return {
    familia, atributos, confianca_global: confiancaGlobal, status, origem_global: origemGlobal,
    regras_aplicadas: aplicadas, regras_rejeitadas: rejeitadas, conflitos,
    evidencias: [...new Set(evidenciasGlobais)], motivo_bloqueio: motivo,
  };
}
